...
=============
PURPOSE
    The "Generate Response" stage of your workflow. Every string the user ever
    sees is built here -- no message text is written inside bot.py.

    Why separate it? Because wording and layout change far more often than
    logic does. Keeping presentation in one file means you can restyle the
    whole bot without any risk of breaking the Telegram plumbing.

    All output uses Telegram's HTML parse mode (<b>, <i>, <code>) because it is
    far more forgiving than MarkdownV2, which requires escaping a long list of
...
# ---------------------------------------------------------------------------
def _job_block(row: pd.Series) -> str:
    """
...
# ---------------------------------------------------------------------------
def category_message(category_key: str, rows: pd.DataFrame) -> str:
    """
...
# ---------------------------------------------------------------------------
def pending_message(df: pd.DataFrame) -> str:
    """
    ...
    for _key, meta in config.CATEGORIES.items():
        rows = outstanding[
