...
======
PURPOSE
    The Telegram layer -- and nothing else. It receives commands, calls
    excel_loader for data, calls formatters for text, and sends the reply.

    Deliberately thin: if you ever want to expose the same data through a web
    page or an email digest, you reuse excel_loader.py and formatters.py and
    only rewrite this file.
...
def _split_message(text: str, limit: int = MAX_MESSAGE_LENGTH - 100) -> list[str]:
    ...
    Splits in three escalating stages, so the result is always under the limit
    no matter what shape the text is:

      1. On blank lines (the gap between job blocks). Preferred, because a job
         is never cut in half and no <b> tag is left unclosed.
      2. On single newlines, for a block with no blank lines inside it -- e.g.
         one category in /pending holding a hundred outstanding jobs.
      3. On raw character count, as a last resort for a single line longer than
         the limit (a pathological Job Description pasted from somewhere).

    Without stages 2 and 3 an oversized block would pass through unsplit and
    ...
    def pack(pieces: list[str], joiner: str) -> list[str]:
        """Greedily fill chunks up to the limit, rejoining with `joiner`."""
        out, current = [], ""
        for piece in pieces:
    ...
    # Stage 2: any chunk still too long gets broken on single newlines.
    refined = []
    for chunk in chunks:
        refined.extend(pack(chunk.split("\n"), "\n") if len(chunk) > limit else [chunk])

    # Stage 3: hard slice anything that is still oversized.
    final = []
...
async def _reply(update: Update, text: str, keyboard: InlineKeyboardMarkup | None = None):
    ...
    if chat is None:
        logger.warning("Update had no chat to reply to; skipping.")
        return
...
async def unknown_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    NOTE: uses effective_message, NOT update.message. When a user EDITS a
    message they already sent, Telegram delivers it as `edited_message` and
    `update.message` is None -- calling .reply_text() on it raises
    AttributeError. effective_message resolves to whichever one is present.
    """
